...
public class ByStreamService {
    ...
    public ContentDto getRecommendationsForNewUser(String university, String stream, Integer sem) {
        try{
            List<Post> allPosts = findPostByEducation(university, stream, sem);
            System.out.println("Show allpost list"+allPosts);
            Map<Post, Double> postWeight = calculatePostWeights(allPosts, university, stream);
    ...
    public ContentDto getRecommendationsForExistingUser(String university, String stream, Integer sem) {
        ...
        Map<Post, Double> postWeight = calculatePostWeights(filteredPosts, university, stream);
    ...
    private List<Post> findPostByEducation(String university, String stream, Integer sem) {
        ...
        List<UserProfile> uniqueUserProfiles = new ArrayList<>();
                ...
                List<College> allCollege = collegeRepository.findByUniversityName(university);
        System.out.println("Show allcollege list"+allCollege);

                for (College clg : allCollege) {
                    Education edu = clg.getEducation();
                    if(edu.getSpecializationStream().equals(stream) && edu.getCurrentSemester()==sem){
    ...
    public List<Post> getNotesByUniversity(String university, String stream, Integer semester){
        List<UserProfile> uniqueUserProfiles = new ArrayList<>();
        List<College> allCollege = collegeRepository.findByUniversityName(university);
        for (College clg : allCollege) {
            Education edu = clg.getEducation();
            if(edu.getSpecializationStream().equals(stream) && edu.getCurrentSemester() == semester){
                uniqueUserProfiles.add(edu.getUser());
            }
        }
        List<Post> allPosts = new ArrayList<>();
    ...
    public Map<String, List<Post>> getQuestionPapersByUniversity(String university, String stream, Integer semester){
        List<UserProfile> uniqueUserProfiles = new ArrayList<>();
        List<College> allCollege = collegeRepository.findByUniversityName(university);
        for (College clg : allCollege) {
            Education edu = clg.getEducation();
    ...
    public List<Post> getWriteUpsByUniversity(String university, String stream, Integer semester){
        List<UserProfile> uniqueUserProfiles = new ArrayList<>();
        List<College> allCollege = collegeRepository.findByUniversityName(university);
        for (College clg : allCollege) {
            Education edu = clg.getEducation();
